<template>
  <q-card flat style="min-width:60%">
    ...
    <q-card-section class="q-pa-none">
      <q-form ref="form" class="page-color">
        ...
        <q-separator /> -->
        <!-- <q-tab-panel name="inf" id="tab-inf" class="scroll card-scroll__content-add-tab">  calc(100vh - 102px)-->

        <div class="row">
          <div class="col q-mr-xl">
            <q-scroll-area ref="scrollAreaRef" style="height:calc(100vh - 102px);" @scroll="onScrollTab">
              <q-card class="q-ma-md" id="basic-inf">
                <q-card-section class="q-pb-none">
                  <div class="text-subtitle2">{{$t('tabs.basicInf')}}</div>
                </q-card-section>
                <q-card-section>
                  <div class="row q-gutter-xs">
                    <div class="col-12">
                      <select-category v-model="selectedCategory" v-model:parents="selectedCategories" :categories="categories" option-value="_id"
                                       :rules="[v=>v&&v.length>0||$t('error.required')]" :expanded="expanded" option-label="label" :maximized="false"
                  ...
                  <div class="row q-gutter-xs">
                    <div class="col-12 col-md-5">
                      <q-input v-model.trim="data.title" v-upperCaseFirst :dense="$store.getters.dense.input" :label="`*${$t('product.name')}`"
                               counter maxlength="120" :rules="[v=>v&&v.length>0||$t('error.required')]">
                        <template v-if="data.title&&data.title.length" v-slot:append>
                          <q-icon size="18px" name="close" class="cursor-pointer" @click="data.title=null" />
                        </template>
                      </q-input>
                    </div>
                    <q-space />
                    <div class="col-12 col-md-5">
                      <q-input v-model.trim="data.code" v-uppercase debounce="500" :dense="$store.getters.dense.input"
                               :label="`*${$t('product.code')}`" counter maxlength="20"
                               :rules="[v=>v&&v.length>0||$t('error.required'),v=>checkExistCode(v)]">
                        <template v-if="data.code&&data.code.length" v-slot:append>
                          <q-icon size="18px" name="close" class="cursor-pointer" @click="data.code=null" />
                        </template>
                      </q-input>
                    </div>
                  </div>
                  <div class="row q-gutter-sm q-mb-lg">
                    <div class="col-12">
                      <q-input v-model.trim="data.desc" autogrow :dense="$store.getters.dense.input" counter maxlength="3000"
                               :label="`*${$t('global.desc')}`" :rules="[v=>v&&v.length>0||$t('error.required')]" />
                    </div>
                  </div>
              ...
              <q-card class="q-ma-md" id="details-inf">
                <q-card-section class="q-pb-none">
                  <div class="text-subtitle2">{{$t('tabs.detailsInf')}}</div>
                </q-card-section>
                <q-card-section>
                  <div class="row q-gutter-xs">
                    <div class="col-12 col-md-5">
                      <q-input v-model.trim="data.brand" :dense="$store.getters.dense.input" :label="$t('product.brand')" />
                    </div>
                    <q-space />
                    <div class="col-12 col-md-5">
                      <q-input v-model.trim="data.originName" :dense="$store.getters.dense.input" :label="$t('product.originName')" />
                    </div>
                  </div>
                  <div class="row q-gutter-xs">
                    <div class="col-12 col-md-5">
                      <q-input v-model.trim="data.originAddress" :dense="$store.getters.dense.input" :label="$t('product.originAddress')" />
                    </div>
                    <q-space />
                    <div class="col-12 col-md-5">
                  ...
                  <div class="row q-gutter-xs">
                    <div class="col-12 col-md-5">
                      <q-input v-model.trim="data.weight" :dense="$store.getters.dense.input" :label="$t('product.weight')" />
                    </div>
                    <q-space />
                    <div class="col-12 col-md-5">
              ...
              <q-card class="q-ma-md" id="sales-inf">
                <q-card-section class="q-pb-none">
                  <div class="text-subtitle2">{{$t('tabs.salesInf')}}</div>
                </q-card-section>
                <q-card-section>
                  <div v-if="!productTypeData.length" class="row q-gutter-xs">
                    <div class="col-12 col-md-2">
                      {{$t('product.type')}}
                    </div>
                    <div class="col-12 col-md-6">
                      <q-btn outline no-caps color="primary" :label="$t('product.addGroupType')" @click="onAddGroupTypes" />
                    </div>
                    <q-space />
                  </div>
                  <div v-if="!productTypeData.length" class="row q-gutter-xs">
                    <div class="col-12 col-md-2">{{$t('product.priceSale')}}</div>
                    <div class="col-12 col-md-6">
                  ...
                  <div v-if="!productTypeData.length" class="row q-gutter-xs">
                    <div class="col-12 col-md-2">{{$t('product.priceImport')}}</div>
                    <div class="col-12 col-md-6">
                      <q-input v-model="data.priceImport" type="number" :dense="$store.getters.dense.input"
                  ...
                  <div v-if="!productTypeData.length" class="row q-gutter-xs">
                    <div class="col-12 col-md-2">{{$t('product.quantityStore')}}</div>
                    <div class="col-12 col-md-6">
                  ...
                  <div v-if="productTypeData.length" class="row q-gutter-xs">
                    <div class="col-12 col-md-2">{{$t('product.typeOne')}}</div>
                    <div class="col-12 col-md-9 q-pa-md q-mb-md" style="background-color:#f6f6f6f6;">
                      <div class="row q-gutter-xs">
                        <div class="col-2">{{$t('product.typeName')}}</div>
                        <div class="col-9">
                          <q-input v-model.trim="productTypeData[0].label" :dense="$store.getters.dense.input"
                                   :label="`*${$t('product.typeName')}`" />
                        </div>
                      </div>
                      <div class="row q-gutter-xs">
                        <div class="col-2">{{$t('product.type')}}</div>
                        <div class="col-9">
                          <div v-for="(e,i) in productTypeData[0].options" :key="i">
                            <q-input v-model.trim="productTypeData[0].options[i]"
                                     :dense="$store.getters.dense.input" :label="`*${$t('product.type')}`" />
                          </div>
                        </div>
                      </div>
                      <div class="row q-gutter-xs q-mt-md">
                        <div class="col-2"></div>
                        <div class="col-9">
                          <q-btn outline no-caps color="primary" :label="$t('product.addType')" @click="onAddTypeOptions(0)" />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div v-if="productTypeData.length" class="row q-gutter-xs">
                    <div class="col-12 col-md-2">{{$t('product.typeTwo')}}</div>
                    <div v-if="productTypeData.length===1" class="col-12 col-md-9">
                      <q-btn outline no-caps color="primary" icon="add" :label="$t('global.add')" @click="onAddGroupTypes" />
                    </div>
                    <div v-if="productTypeData.length>1" class="col-12 col-md-9 q-pa-md q-mb-md" style="background-color:#f6f6f6f6;">
                      <div class="row q-gutter-xs">
                        <div class="col-2 seft-center">{{$t('product.typeName')}}</div>
                        <div class="col-9">
                          <q-input v-model.trim="productTypeData[1].label" :dense="$store.getters.dense.input"
                                   :label="`*${$t('product.typeName')}`" />
                        </div>
                      </div>
                      <div class="row q-gutter-xs">
                        <div class="col-2">{{$t('product.type')}}</div>
                        <div class="col-9">
                          <q-input v-model.trim="productTypeData.types" :dense="$store.getters.dense.input" :label="`*${$t('product.type')}`" />
                        </div>
                      </div>
                      <div class="row q-gutter-xs q-mt-md">
                        <div class="col-2"></div>
                        <div class="col-9">
                          <q-btn outline no-caps color="primary" :label="$t('product.addType')" />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div v-if="productTypeData.length" class="row q-gutter-xs q-mb-md">
                    <div class="col-12 col-md-2">{{$t('global.quickConfig')}}</div>
                    <div class="col-12 col-md-9">
                      <div class="row">
                        <div class="col">
                          <q-input v-model="quickConfig.price" :dense="$store.getters.dense.input" :label="$t('product.priceSale')" />
                        </div>
                        <div class="col">
                          <q-input v-model="quickConfig.priceImport" :dense="$store.getters.dense.input" :label="$t('product.priceImport')" />
                        </div>
                        <div class="col">
                          <q-input v-model="quickConfig.quantity" :dense="$store.getters.dense.input" :label="$t('global.quantity')" />
                        </div>
                        <div class="col-2">
                          <q-btn no-caps color="primary" :label="$t('global.apply')" class="q-btn--square" />
                        </div>
                      </div>
                    </div>
                  </div>
                  <div v-if="productTypeData.length" class="row q-gutter-xs">
                    <div class="col-12 col-md-2">{{$t('product.typeList')}}</div>
                    <div class="col-12 col-md-9">
                      <table class="q-table">
                        <thead>
                          <tr>
                            <td>{{$t('global.name')}}</td>
                            <td>{{$t('product.priceSale')}}</td>
                            <td>{{$t('product.priceImport')}}</td>
                            <td>{{$t('global.quantity')}}</td>
                          </tr>
                        </thead>
                        <tbody>
                          <tr>
                            ...
                            <td>
                              <q-input v-model="quickConfig.price" :dense="$store.getters.dense.input" :label="$t('product.priceImport')" />
                            </td>
                            <td>
                              <q-input v-model="quickConfig.price" :dense="$store.getters.dense.input" :label="$t('global.quantity')" />
                            </td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  <div class="row q-gutter-xs">
                    <div class="col-12 col-md-2">{{$t('global.unit')}}</div>
                    <div class="col-12 col-md-9">
                      <q-select v-model="data.unit" use-input emit-value map-options hide-selected fill-input input-debounce="200"
                                :dense="$store.getters.dense.input" :options="units" :label="`*${$t('global.unit')}`" @filter="onFilterUnit"
                                option-value="code"
              ...
              <q-card class="q-ma-md" id="orther-inf">
                <q-card-section class="q-pb-none">
                  <div class="text-subtitle2">{{$t('tabs.ortherInf')}}</div>
                </q-card-section>
                <q-card-section>
                  <!-- </q-tab-panel> -->
              ...
              <q-card class="q-ma-md" id="qrcode-inf">
                <q-card-section class="q-pb-none">
                  <div class="text-subtitle2">{{$t('qrCode.qrCode')}}</div>
                </q-card-section>
                <q-card-section>
          ...
          <div v-if="!$q.platform.is.mobile" class="col-auto q-mt-md">
            <q-list>
              <q-item clickable v-ripple :class="tabs==='basic-inf'?'q-item--active':''" @click="onScrollTabClick(0,'basic-inf')">
                <q-item-section>{{$t('tabs.basicInf')}}</q-item-section>
              </q-item>
              <q-item clickable v-ripple :class="tabs==='details-inf'?'q-item--active':''" @click="onScrollTabClick(400,'details-inf')">
                <q-item-section>{{$t('tabs.detailsInf')}}</q-item-section>
              </q-item>
              <q-item clickable v-ripple :class="tabs==='sales-inf'?'q-item--active':''" @click="onScrollTabClick(500,'sales-inf')">
                <q-item-section>{{$t('tabs.salesInf')}}</q-item-section>
              </q-item>
              <q-item clickable v-ripple :class="tabs==='orther-inf'?'q-item--active':''" @click="onScrollTabClick(600,'orther-inf')">
                <q-item-section>{{$t('tabs.ortherInf')}}</q-item-section>
              </q-item>
              <q-item clickable v-ripple :class="tabs==='qrcode-inf'?'q-item--active':''" @click="onScrollTabClick(750,'qrcode-inf')">
                <q-item-section>{{$t('qrCode.qrCode')}}</q-item-section>
              </q-item>
            </q-list>
          </div>
          <div class="col-1"></div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return {
      scrollAreaRef, tabs, form, data, productTypeData, quickConfig, attrKeys, attrValues, categories, pins, units, unitsPrice, selectedCategory, selectedCategories, expanded,
      selectedFileList, viewType, refTMFileList, isDialogUpload, isDialogFileManager, isProductType,
      onFilterAttrKey(val) {
        if (!val) return
        attrKeys.value = []
      ...
      onAddGroupTypes() {
        productTypeData.value.push({ label: null, options: [null] })
      },
      onAddTypeOptions(index) {
        productTypeData.value[index].options.push(null)
      ...
      onScrollTabClick(top, element) {
        scrollAreaRef.value.setScrollPosition('vertical', top, 150)
      ...
      onScrollTab(evt) {
        ...
        // 600 'orther-inf'
        // 750 'qrcode-inf'
        // Height content element
        // const basicInfHeight = document.getElementById('basic-inf').offsetHeight
        // const detailsInfHeight = document.getElementById('details-inf').offsetHeight + basicInfHeight
        // const salesInfHeight = document.getElementById('sales-inf').offsetHeight + detailsInfHeight
        // const ortherInfHeight = document.getElementById('orther-inf').offsetHeight + salesInfHeight
        // const qrcodeInfHeight = document.getElementById('qrcode-inf').offsetHeight + ortherInfHeight
        // const fixHeight = 300
        // if (evt.verticalPosition > basicInfHeight - fixHeight && evt.verticalPosition < detailsInfHeight - fixHeight) tabs.value = 'details-inf'
        // else if (evt.verticalPosition > detailsInfHeight - fixHeight && evt.verticalPosition < salesInfHeight - fixHeight) tabs.value = 'sales-inf'
        // else if (evt.verticalPosition > salesInfHeight - fixHeight && evt.verticalPosition < ortherInfHeight - fixHeight) tabs.value = 'orther-inf'
        // else if (evt.verticalPosition > ortherInfHeight - fixHeight) tabs.value = 'qrcode-inf'
        // else tabs.value = 'basic-inf'
        if (evt.verticalPosition > 399 && evt.verticalPosition < 500) tabs.value = 'details-inf'
        else if (evt.verticalPosition > 499 && evt.verticalPosition < 600) tabs.value = 'sales-inf'
        else if (evt.verticalPosition > 599 && evt.verticalPosition < 750) tabs.value = 'orther-inf'
        else if (evt.verticalPosition > 749) tabs.value = 'qrcode-inf'
        else tabs.value = 'basic-inf'
      }
    }
  }
