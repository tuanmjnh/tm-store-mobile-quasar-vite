<template>
  <q-form ref="form" class="settings">
    <q-list class="scroll" style="height:calc(100vh - 99px)">
      ...
      <q-separator spaced />
      <q-item-label header>{{$t('setting.dialog')}}</q-item-label>
      <q-item clickable v-ripple>
        <q-item-section>{{$t('global.add')}}</q-item-section>
        <q-item-section side>
          <q-toggle v-model="dialogAdd" class="text-no-wrap" :dense="$store.getters.dense.input" />
        </q-item-section>
      </q-item>
      <q-item clickable v-ripple>
        <q-item-section>{{$t('global.edit')}}</q-item-section>
        <q-item-section side>
          <q-toggle v-model="dialogEdit" class="text-no-wrap" :dense="$store.getters.dense.input" />
        </q-item-section>
      </q-item>
      <q-item clickable v-ripple>
        <q-item-section>{{$t('global.import')}}</q-item-section>
        <q-item-section side>
          <q-toggle v-model="dialogImport" class="text-no-wrap" :dense="$store.getters.dense.input" />
        </q-item-section>
      </q-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    const dialogAdd = computed({
      get: () => $store.getters.dialog.add,
      set: val => {
        $store.commit('settings/SET_DIALOG', { 'add': val })
        onUpdate()
      }
    })
    const dialogEdit = computed({
      get: () => $store.getters.dialog.edit,
      set: val => {
        $store.commit('settings/SET_DIALOG', { 'edit': val })
        onUpdate()
      }
    })
    const dialogImport = computed({
      get: () => $store.getters.dialog.import,
      set: val => {
        $store.commit('settings/SET_DIALOG', { 'import': val })
        onUpdate()
      }
    })
    ...
    onInit()
    return {
      form, darkMode, languages, language, dialogAdd, dialogEdit, dialogImport, denseForm, denseTable, denseInput, denseButton, denseMenu, shadowTable,
